from simulate import Algorithm, ColorCode, NoiseModel, CodeFactory, StimSimulator, LogicalCircuit
import pandas as pd
import numpy as np
import torch
...
class ColorCodeDataset(IterableDataset):
    """
    ...
    def __init__(self, d=3, rounds=10, p=2.0, batch_size=32, 
                 stage_manager=None, num_workers=8, global_step_offset=0, **kwargs):
        """
        Args:
            d: Color code parameter d
    ...
    def _build_static_graph_structure(self):
        """Build the static color code graph structure (independent of rounds)."""
        # Create color code metadata
        cf = CodeFactory(ColorCode, {'d': self.d})
        metadata = cf().metadata
        faces = pd.DataFrame(metadata.entities['face'])
        faces2 = faces.merge(faces, how='cross', suffixes=("_1", "_2"))

        dist = np.sqrt((faces2['pos_x_1'] - faces2['pos_x_2'])**2 + (faces2['pos_y_1'] - faces2['pos_y_2'])**2)
        faces2 = faces2[np.isclose(dist, dist[dist > 0].min())]
        dx, dy = faces2['pos_x_1'] - faces2['pos_x_2'], faces2['pos_y_1'] - faces2['pos_y_2']
        angle = np.round(np.arctan2(dy, dx) * 180/np.pi).astype(int)
        _, idxs = np.unique(angle, return_inverse=True)
        faces2['edge_type'] = idxs
        faces2 = faces2[['face_id_1', 'face_id_2', 'edge_type']]
        faces2 = faces2.sort_values(by=['face_id_1', 'edge_type']).reset_index(drop=True)
        return faces2
...
if __name__ == '__main__':
    ds = ColorCodeDataset(d=7, rounds=3)
    print(ds._build_static_graph_structure())

    alg = Algorithm.build_memory(cycles=4)
    cf = CodeFactory(ColorCode, {'d': 5})
    lc, _, mt = LogicalCircuit.from_algorithm(alg, cf)
    print(pd.DataFrame(mt.detectors))